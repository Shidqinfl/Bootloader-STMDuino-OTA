...
void otadrive_ota::setBaseURL(String url)
{
    _baseURL = url;
}

String otadrive_ota::getBaseURL()
{
    return _baseURL;
}
...
{
    String ChipIdHex = "0" + String(*(uint32_t*)(UID_BASE), HEX) + String(*(uint32_t*)(UID_BASE+0x08), HEX);
    return ChipIdHex;
}
...
bool otadrive_ota::download(Client &client, String url, File *file, String *outStr)
{
    ...
    if (http.get(url))
    {
        ...
        if (http.resp_code == 200)
        {
            if (file)
            {
                ...
                while (millis() - t0 < 15000 && rem > 0)
                {
                    ...
                    rem -= rd;
                    DEBUG_SERIAL.printf("%d/%d rem : %s", rem, http.total_len, file->name());
                }

                DEBUG_SERIAL.printf("%d/%d bytes downloaded and writted to file : %s", rem, http.total_len, file->name());
                file->close();
                return true;
            }
        ...
        {
            DEBUG_SERIAL.printf("downloaded error code %d, %s", http.resp_code, http.client.readString().c_str());
        }
    }
...
bool otadrive_ota::syncResources(Client &client)
{
    if (fileObj == nullptr)
    {
        DEBUG_SERIAL.printf("file system doesn't set, call setFileSystem() in setup");
        return false;
    }
    ...
    while (list.length())
    {
        ...
        DEBUG_SERIAL.printf("file data: %s, MD5=%s", fn.c_str(), md5.c_str());
        ...
        if (md5_local == md5)
        {
            DEBUG_SERIAL.printf("local MD5 is match for %s", fn.c_str());
            continue;
        }
        else
        {
            // lets download and replace file
            DEBUG_SERIAL.printf("MD5 not match for %s, lets download", fn.c_str());
            File file = fileObj->open(fn, "w+");
            if (!file)
            {
                DEBUG_SERIAL.printf("Faild to create file\n");
                return false;
            }
 ...
 */
// bool otadrive_ota::sendAlive(TinyGsmClient &client)
// {
...
// TODO: add watchdog reset
FotaResult otadrive_ota::updateFirmware(Client &client, bool reboot)
{
    updateInfo inf = updateFirmwareInfo(client);
    if (!inf.available)
    {
        DEBUG_SERIAL.printf("No new firmware available");
    ...
    case FOTA_UPDATE_FAILED:
        DEBUG_SERIAL.printf("HTTP_UPDATE_FAILED Error");
        break;

    case FOTA_UPDATE_NO_UPDATES:
        DEBUG_SERIAL.printf("HTTP_UPDATE_NO_UPDATES");
        break;
    ...
    case FOTA_UPDATE_OK:
    {
        Version = inf.version;
        sendAlive(client);
        DEBUG_SERIAL.printf("HTTP_UPDATE_OK");
        if (reboot)
            // ESP.restart();
    ...
    default:
        DEBUG_SERIAL.printf("HTTP_UPDATE_CODE: %d", ret);
        break;
    }
 ...
 */
// FotaResult otadrive_ota::updateFirmware(bool reboot)
// {
//     WiFiClient client;
...
updateInfo otadrive_ota::updateFirmwareInfo(Client &client)
{
    ...
    if (!http.get(url, 0, 0))
    {
        DEBUG_SERIAL.printf("http error\n");
        return inf;
    }

    DEBUG_SERIAL.printf("heads [%d] \n", (int)http.resp_code);
    ...
    {
    case 0:
        DEBUG_SERIAL.printf("connect error\n");
        inf.code = update_result::ConnectError;
        break;

    case 404:
        // HTTP_CODE_NOT_FOUND
        DEBUG_SERIAL.printf("not found error\n");
        inf.code = update_result::NoFirmwareExists;
        break;

    case 304:
        // HTTP_CODE_NOT_MODIFIED
        DEBUG_SERIAL.printf("no new firmware\n");
        inf.code = update_result::AlreadyUpToDate;
        break;

    case 401:
        // HTTP_CODE_UNAUTHORIZED
        DEBUG_SERIAL.printf("unauthorized error\n");
...
OTAdrive_ns::KeyValueList otadrive_ota::getConfigValues(Client &client)
{
    ...
    DEBUG_SERIAL.printf("config download: %s\n", conf.c_str());
