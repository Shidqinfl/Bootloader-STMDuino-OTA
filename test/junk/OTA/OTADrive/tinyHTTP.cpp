...
bool TinyHTTP::begin_connect(const String &url)
{
    ...
    // Read host from url
    int host_end = u.indexOf('/');
    if (host_end == -1)
        host_end = u.length();
    host = u.substring(0, host_end);

    // Read port from url
    int port_ind = host.indexOf(':');
    if (port_ind != -1)
    {
        port = host.substring(port_ind + 1).toInt();
        host = host.substring(0, port_ind);
    }

    // Read uri from url
    uri = u.substring(host_end);
